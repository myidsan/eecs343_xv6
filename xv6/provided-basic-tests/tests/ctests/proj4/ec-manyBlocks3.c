/* call tagFile 90 times with 600-char values.  
...
main(int argc, char *argv[])
{
   ...
   value[len] = 0; // null termination



   // give the file 90 tags
   int numTags = 90;
   for(i = 0; i < numTags; i++){
      key[8] = (char)(i + 1);     // set the last character before the NULL
      value[len - 1] = (char)(i + 1);  // set the last character before the NULL
   ...
   // get the file tag 90 times
   for(i = 0; i < numTags; i++){
      key[8] = (char)(i + 1);
      valueLength = getFileTag(fd, key, buf, 600);
